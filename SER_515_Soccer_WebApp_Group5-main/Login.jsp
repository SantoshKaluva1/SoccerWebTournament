<html>
    ...
    <style>
body {
 background-image: url(css/loginImage.jpeg) !important;;
}
</style>
        <title>
           Login Page 
...
Class.forName("com.mysql.jdbc.Driver");
Connection conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/UserDB","root","Ramesh@52");
String sql = "select UserName from user where Approved!='True' and UserName!='akhil'";
PreparedStatement ps = conn.prepareStatement(sql);
ResultSet rs = ps.executeQuery();
...
for (String a: teamsNames)
{
  %>
  <tr>
      <td><%= a %></td>
      <td><button class="approve">Approve</button></td>
      <td><button class="deny">Deny</button></td>
  </tr>
<%
}
%>
